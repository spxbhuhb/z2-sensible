...
class PluginAnnotationsProvider(testServices: TestServices) : EnvironmentConfigurator(testServices) {
    ...
    companion object {
        private const val ANNOTATIONS_JAR_DIR = "../z2-sensible-runtime/build/libs/"
        private val ANNOTATIONS_JAR_FILTER = FilenameFilter { _, name ->
            name.startsWith("z2-sensible-runtime-jvm") && name.endsWith(".jar") && "sources" !in name
        }
    }
    ...
    private val failMessage = { "Jar with annotations does not exist. Please run :z2-sensible-runtime:jar" }
}